...
def chunk_text(text: str, chunk_chars: int, overlap: int = 0) -> List[Tuple[int, int, str]]:
    """
    Return list of (start, end, chunk_str). Overlap is in characters.
    """
    if chunk_chars <= 0:
...
def main() -> None:
    ...
    ap.add_argument("--limit", type=int, default=5, help="Max number of XML files to process.")
    ...
    ap.add_argument("--chunk-chars", type=int, default=4000, help="Chunk size in characters (default: 4000). Use 0 to disable.")
    ap.add_argument("--chunk-overlap", type=int, default=200, help="Chunk overlap in characters (default: 200).")
    ap.add_argument("--max-chunks-per-doc", type=int, default=10, help="Safety cap per document (default: 10).")
    args = ap.parse_args()
    ...
    with out_path.open("w", encoding="utf-8") as f:
        for fp in files:
            if n_written >= args.limit and args.chunk_chars == 0:
                break
            # If chunking is ON, --limit refers to documents, not lines
            if args.chunk_chars != 0 and (n_written // max(1, args.max_chunks_per_doc)) >= args.limit:
                break
            ...
            for chunk_id, (a, b, chunk) in enumerate(chunks):
                ...
                row: Dict[str, object] = {
                    ...
                    # keep for now; later we can add a flag to drop these for scale
                    "tokens": res.tokens,
                    "lemmas": res.lemmas,
                    "matched_forms": res.matched_forms,
                    "matched_lemmas": res.matched_lemmas,
                }

                f.write(json.dumps(row, ensure_ascii=False) + "\n")

            # progress by documents, not by lines
            if args.chunk_chars == 0:
                n_written += 1

    print(f"[OK] wrote {out_path} scanned={scanned} (chunk_chars={args.chunk_chars})")
